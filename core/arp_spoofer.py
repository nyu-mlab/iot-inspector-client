...
def spoof_internet_traffic():
    ...
    device_list = global_state.arp_cache.get_mac_ip_list()

    common.log(f'[ARP Spoofer] Spoofing internet traffic for {len(device_list)} devices')

    # Send ARP spoofing packets for each inspected device
    for (device_mac_addr, device_ip_addr) in device_list:
        # Make sure that the device is in the ARP cache; if not, skip
        try:
            send_spoofed_arp(device_mac_addr, device_ip_addr, gateway_mac_addr, gateway_ip_addr)
        except Exception:
            common.log(f'[ARP Spoofer] Error spoofing {device_mac_addr}, {device_ip_addr} <-> {gateway_mac_addr}, {gateway_ip_addr}, because\n' + traceback.format_exc())
