...
# Counts the number of outgoing packets per device. Maps device_mac_addr -> {ts -> packet_count}
outgoing_packet_counter_dict = dict()

# Counts the number of outgoing bytes per device. Maps device_mac_addr -> {ts -> byte_count}
outgoing_byte_counter_dict = dict()

# Records recent hostnames for each device. Maps device_mac_addr -> {hostname -> ts}
recent_hostnames_dict = dict()
